...
function setupFdk(data, syncInitialization) {
  ...
  const promiseInit = extension.initialize(data).catch((err) => {
    logger.error(err);
    throw err;
  });
  let router = setupRoutes(extension);
  let { apiRoutes, applicationProxyRoutes, partnerApiRoutes } =
  ...
  return syncInitialization
    ? promiseInit.then(() => configInstance).catch(() => configInstance)
    : configInstance;
}
